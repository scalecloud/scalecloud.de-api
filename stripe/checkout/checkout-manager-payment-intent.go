...
import (
	...
	"github.com/scalecloud/scalecloud.de-api/stripe/price"
	"github.com/scalecloud/scalecloud.de-api/stripe/product"
...
func CreateCheckoutSubscription(c context.Context, token string, checkoutIntegrationRequest CheckoutPaymentIntentRequest) (CheckoutPaymentIntentReply, error) {
	...
	price, err := price.GetPrice(c, checkoutIntegrationRequest.ProductID)
	if err != nil {
...
func UpdateCheckoutSubscription(c context.Context, token string, checkoutIntegrationUpdateRequest CheckoutPaymentIntentUpdateRequest) (CheckoutPaymentIntentUpdateReply, error) {
	...
	customerIDFromUID, err := customermanager.GetCustomerIDByUID(c, tokenDetails.UID)
	if err != nil {
	...
	sub, err := subscriptionmanager.GetSubscriptionByID(c, checkoutIntegrationUpdateRequest.SubscriptionID)
	if err != nil {
...
func GetCheckoutProduct(c context.Context, token string, checkoutProductRequest CheckoutProductRequest) (CheckoutProductReply, error) {
	...
	subscription, err := subscriptionmanager.GetSubscriptionByID(c, checkoutProductRequest.SubscriptionID)
	if err != nil {
		logger.Error("Error getting subscription", zap.Error(err))
		return CheckoutProductReply{}, err
	}
	...
	price, err := price.GetPrice(c, productID)
	if err != nil {
		logger.Error("Error getting price", zap.Error(err))
		return CheckoutProductReply{}, err
	}
	...
	product, err := product.GetProduct(c, productID)
	if err != nil {
